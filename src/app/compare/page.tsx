    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center">
        ...
        <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-2">
          Compare karne ke liye gadiyan chunein!
        </h2>
            ...
            {compareList.map((car, index) => (
              <React.Fragment key={car.id || index}>
                <div className="text-center flex-1 min-w-[200px] group relative">
                  <div className="relative h-32 md:h-48 w-full mb-6">
                    {/* ✅ FIXED: Next.js Image replace kiya */}
                    <Image
                  ...
                  <div className="space-y-1">
                    <p className="text-blue-600 font-black text-[10px] uppercase tracking-widest">
                      {car.sourceTable || "Featured"}
                    </p>
                    <h2 className="text-lg md:text-xl font-black text-gray-900 leading-tight line-clamp-1">
                      {car.name}
                    </h2>
            ...
            <table className="w-full text-left table-fixed min-w-[900px]">
              <thead>
                <tr className="bg-gray-50/50">
                  ...
                  {compareList.map((car, idx) => (
                    <th key={idx} className="p-6 font-black text-gray-900 text-center text-md border-l border-gray-50">
                      {car.name}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-gray-100">
                <tr className="hover:bg-blue-50/40 transition-colors">
                  ...
                  {compareList.map((car, idx) => (
                    <td key={idx} className="p-6 text-center font-bold text-gray-800 border-l border-gray-50">
                      {car.fuelType || "Petrol"}
                    </td>
                  ))}
                </tr>
                <tr className="hover:bg-blue-50/40 transition-colors">
                  ...
                  {compareList.map((car, idx) => (
                    <td key={idx} className="p-6 text-center font-black text-blue-600 text-lg border-l border-gray-50">
                      {car.priceRange}
                    </td>
                  ))}
                </tr>
                <tr className="hover:bg-blue-50/40 transition-colors">
                  ...
                  {compareList.map((car, idx) => (
                    <td key={idx} className="p-6 text-center font-bold text-gray-800 border-l border-gray-50">
                      {car.specs?.transmission || "Manual / AMT"}
                    </td>
                  ))}
                </tr>
                <tr className="hover:bg-blue-50/40 transition-colors">
