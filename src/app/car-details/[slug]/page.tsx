...
import UserReviews from '@/components/UserReviews'; // ✅ Reviews Component
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 font-sans">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
            ...
            <div className="flex flex-col h-full"> 
                ...
                <div className="flex gap-4 mt-4 w-full">
                    <button onClick={handleOpenOffers} className="flex-1 py-3.5 border-2 border-red-600 text-red-600 font-bold rounded hover:bg-red-50 transition-colors uppercase text-sm tracking-wide">
                        {isUsed ? "Check Warranty" : "Check Offers"}
                    </button>
                    <button onClick={handleOpenBooking} className="flex-1 py-3.5 bg-red-700 text-white font-bold rounded hover:bg-red-800 transition-colors shadow-lg shadow-red-100 uppercase text-sm tracking-wide">
                        {isUsed ? "Contact Seller" : "Book Visit"}
                    </button>
                </div>
                {isUsed && (
                     <button onClick={handleWhatsApp} className="w-full mt-3 border border-green-500 text-green-600 font-bold py-3 rounded hover:bg-green-50 transition flex items-center justify-center gap-2">
        ...
        {/* --- TABS SECTION (SPECS / FEATURES / REVIEWS) --- */}
        <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="flex border-b border-gray-200 mb-8 overflow-x-auto">
                <button onClick={() => setActiveTab('specs')} className={`pb-3 pr-8 text-sm md:text-base font-bold transition-all border-b-2 whitespace-nowrap ${activeTab === 'specs' ? 'border-red-500 text-gray-900' : 'border-transparent text-gray-400'}`}>
                    {isUsed ? "Car Overview" : "Key Specifications"}
                </button>
                <button onClick={() => setActiveTab('features')} className={`pb-3 px-8 text-sm md:text-base font-bold transition-all border-b-2 whitespace-nowrap ${activeTab === 'features' ? 'border-red-500 text-gray-900' : 'border-transparent text-gray-400'}`}>
                    {isUsed ? "Condition & Features" : "Top Features"}
                </button>
                {/* ✅ REVIEWS TAB */}
                <button onClick={() => setActiveTab('reviews')} className={`pb-3 px-8 text-sm md:text-base font-bold transition-all border-b-2 whitespace-nowrap ${activeTab === 'reviews' ? 'border-red-500 text-gray-900' : 'border-transparent text-gray-400'}`}>
                    User Reviews
                </button>
            </div>
            ...
            {/* ✅ Tab 3: REVIEWS (THIS IS WHAT WAS MISSING) */}
            {activeTab === 'reviews' && (
