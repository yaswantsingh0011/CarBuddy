...
const ElectricCarCard: React.FC<any> = (props) => {
  // Destructure props safely (Expert Verdict hata diya hai yahan se)
  const { id, name, priceRange, imageUrl, images, fuelType, slug, onOfferClick, onDetailClick } = props;
  ...
  const handleCompareToggle = (e: React.MouseEvent) => {
    ...
    if (inCompare) removeFromCompare(id || name);
    else addToCompare({ id, name, priceRange, imageUrl, images, fuelType, slug });
  };

  const handleShortlistClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (toggleShortlist) toggleShortlist({ id, name, priceRange, imageUrl, images, fuelType, slug });
  };
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-lg transition-all relative flex flex-col h-full group cursor-pointer" onClick={onDetailClick}>
      ...
      <div className="relative h-48 w-full bg-gray-100 overflow-hidden">
        <Image src={finalImg} alt={name || "Car"} fill className="object-cover group-hover:scale-105 transition-all" unoptimized />
        
        <button onClick={handleCompareToggle} className={`absolute top-3 left-3 w-8 h-8 rounded-full flex items-center justify-center shadow-md z-10 transition-all ${inCompare ? 'bg-blue-600 text-white' : 'bg-white/90 text-gray-700 hover:text-blue-600'}`}>
          <FaExchangeAlt className="text-sm" />
        </button>

        <button onClick={handleShortlistClick} className="absolute top-3 right-3 w-8 h-8 bg-white/90 rounded-full flex items-center justify-center shadow-md z-10">
      ...
      <div className="p-4 flex flex-col flex-grow space-y-3">
        ...
        {/* Verdict Box Removed - Original Design Restored */}

        <div className="mt-auto grid grid-cols-2 gap-2 pt-2">
          <button className="border border-blue-600 text-blue-600 py-2 rounded-lg font-bold text-xs uppercase hover:bg-blue-50 transition-colors">Details</button>
          <button className="bg-blue-600 text-white py-2 rounded-lg font-bold text-xs uppercase flex items-center justify-center gap-1 hover:bg-blue-700 transition-colors" onClick={(e) => { e.stopPropagation(); onOfferClick?.(); }}>
            <FaTag /> Offers
          </button>
        </div>
      </div>
