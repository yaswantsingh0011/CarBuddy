...
interface OnRoadPriceModalProps {
  ...
  carName: string;
  price: string | number;
  city?: string; // Optional kiya hai taaki crash na ho
  onOpenEMI: () => void;
  onOpenBooking: () => void;
}

const OnRoadPriceModal: React.FC<OnRoadPriceModalProps> = ({ 
  isOpen, onClose, carName, price, city = "Jaipur", 
  onOpenEMI, onOpenBooking 
}) => {
  ...
  const isLuxury = exShowroom > 2000000;
  
  const rtoPercent = isLuxury ? 0.12 : 0.10;
  const insurancePercent = 0.04;
  ...
  const optionalCharges = Math.round(exShowroom * 0.01); 
